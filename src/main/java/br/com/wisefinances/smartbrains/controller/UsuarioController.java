...
public class UsuarioController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity<MessagesResponseDTO> saveUsuario(@RequestBody @Valid UsuarioDTO UsuarioDTO) {
        return ResponseEntity.status(201).body(usuarioService.save(UsuarioDTO));
    }

    @PatchMapping("/{id}")
    @Transactional
    public ResponseEntity<UsuarioDTO> updateUsuario(@PathVariable Integer id, @RequestBody @Valid UsuarioDTO UsuarioDTO) {
